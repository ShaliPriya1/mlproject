...
for folder_name in os.listdir(root_folder):
    ...
    if os.path.isdir(folder_path):
        # List to hold the folder's document texts
        folder_documents = []
        ...
        for pdf_file in pdf_files:
            ...
            if text:
                ...
                # Store each document's embedding as part of the folder
                documents.setdefault(folder_name, []).append(embedding.tolist())  # Convert numpy array to list

        # Generate and store the folder embedding (using Sentence-BERT for folder name)
        folder_name_embeddings[folder_name] = get_sentence_embedding(folder_name).tolist()  # Convert numpy array to list
